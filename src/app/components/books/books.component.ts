...
export class BooksComponent implements OnInit {
  ...
  orderItems: any = [];
  constructor(private auth: LocalStorageService, private router: Router, private shared: SharedServiceService) { }

  ngOnInit(): void {
    const OrderItems = this.auth.getValue('order_items', true);
    if (OrderItems) {
      this.orderItems = OrderItems;
    }
    for (let i = 0; i < this.allBooks.length; i++) {
      this.allBooks[i].quantity = 1;
    }
  }


  decreaseQuantity(i): any {
    if (this.allBooks[i].quantity > 1) {
      this.allBooks[i].quantity -= 1;
    }
  }
  increaseQuantity(i): any {
    if (this.allBooks[i].quantity) {
      this.allBooks[i].quantity += 1;
      return false;
    }
    this.allBooks[i].quantity = 1;
  ...
  orderNow(item): any {
    this.pushToArray(this.orderItems, item);
    this.auth.setValue('order_items', this.orderItems);
    this.shared.addItemToCart.emit();
      this.router.navigate(['/cart']);
  }

  pushToArray(arr, obj): any {
    const addOn = obj._id + '_' + 32;
    obj.addOn = 0;
    obj.addOn = addOn;
    const index = arr.findIndex((e) => e.addOn === obj.addOn);
    if (arr[index]) {
      if (addOn != arr[index].addOn) {
        arr.push(obj);
      } else {
        const oldQty = arr[index].quantity;
        const newQty = obj.quantity;
        obj.quantity = oldQty + newQty;
        arr[index] = obj;
      }
    } else {
      arr.push(obj);
